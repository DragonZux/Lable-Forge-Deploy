...
export default function DeclineProjectInvitationPage() {
  const { token } = useParams();
  const { declineProject } = useInvitationActions();
  ...
  useEffect(() => {
    const handleDecline = async () => {
      ...
      try {
        await declineProject(token as string);
        setStatus('success');
      } catch (err: any) {
        setStatus('error');
    ...
    <div className="page-shell flex min-h-[70vh] items-center justify-center">
      ...
      {status === 'success' && (
        <div className="flex flex-col items-center animate-in zoom-in duration-300">
          ...
          <h1 className="font-display text-3xl text-foreground">Project invitation declined</h1>
      ...
      {status === 'error' && (
        <div className="flex flex-col items-center animate-in zoom-in duration-300">
          ...
          <h1 className="font-display text-3xl text-foreground">Cannot decline project invitation</h1>
