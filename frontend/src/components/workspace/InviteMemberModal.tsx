...
const ROLES = [
  { id: 'member', label: 'Member', desc: 'Can create projects and upload data' },
  { id: 'admin', label: 'Admin', desc: 'Full control over workspace and members' },
  { id: 'viewer', label: 'Viewer', desc: 'Can only view projects' },
];

export function InviteMemberModal({ workspaceId, workspaceName, isOpen, onClose }: Props) {
  const { sendWorkspaceInvite } = useSendInvitation();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      await sendWorkspaceInvite(workspaceId, { email, role, message });
      setSuccess(true);
    ...
    >
      <div className="space-y-6 pt-2">
        <p className="text-muted-foreground text-sm">
          Invite people to join workspace <span className="text-accent font-semibold">{workspaceName}</span>
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              placeholder="Hi, join us on Label Forge..."
              rows={3}
